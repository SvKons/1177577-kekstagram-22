...
const getPhotoComments = () => {
  ...
  return new Array(commentsTotal).fill(null).map((item, userId) => getComment(userId + 1));
};

const getPublicPhotos = new Array(TOTAL_PHOTOS).fill(null).map((item, userId) => getPhoto(userId + 1));
